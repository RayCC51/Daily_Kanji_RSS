...
with open("dictionary_with_words.csv", mode="r", encoding="utf-8") as f:
    reader = csv.reader(f)
...
for count in split_counts:
    ...
    for i, line in enumerate(days_lines):
        ...
        for idx in today_indices:
            if idx in kanji_db:
                _, hanji, level, buseu, eum, hun, ja_on, ja_kun, ex_words = kanji_db[idx]
                ...
                html_content += f"            <li>예시단어: {ex_words}</li>\n"
                html_content += "          </ul>\n"
                html_content += "          <br/>\n"
